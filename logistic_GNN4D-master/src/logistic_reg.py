...
def extract_spectral_features(adj, k, *, row_norm=True):
    """
    返回特征: [U, ReLU(U)]  -> 形状 (N, 2k)
    - 先做谱分解取前 k 个特征向量 U
    - 对 U 做列符号对齐（避免跨图随机翻符号）
    - 可选行归一化（默认开启）
    - 拼接 U 和 ReLU(U)
    """
    # 解壳
    if isinstance(adj, np.ndarray) and adj.dtype == object and adj.ndim == 0:
        adj = adj.item()
    ...
    # 拉普拉斯 & 特征分解
    L = laplacian(adj, normed=True).astype(np.float64)
    _, U = eigh(L, subset_by_index=(0, k - 1), check_finite=False)
    ...
    # # --- ReLU & 拼接 ---
    # U_relu = np.maximum(U, 0.0)
    # features = np.hstack([U, U_relu])  # (N, 2k)
...
def train_logistic_regression(gen, k, save_path="./models/lr_model.pkl", n_jobs=os.cpu_count()//2):
    # 如果模型已存在 → 直接加载
...
def evaluate_using_logitic(W, true_labels ,lr_model, k):
    ...
    # 计算准确率
    acc_logistic, logistic_best_matched_pred = compute_accuracy_spectral(pred_labels, true_labels, k)

    # Local refinement
    logistic_best_matched_pred = local_refinement_by_neighbors(W, logistic_best_matched_pred, k)
    acc_logistic_refined, _ = compute_accuracy_spectral(logistic_best_matched_pred, true_labels, k)

    return acc_logistic,  acc_logistic_refined
...
def test_single_first_period(gnn_first_period, lr_model, gen, n_classes, args, iter, mode='balanced', class_sizes=None):
    ...
    acc_logistic, acc_refined_logistic = evaluate_using_logitic(W, true_labels, lr_model, n_classes)
    ...
    return loss_test_first, acc_test_first ,acc_spectral_clustering, acc_refined, acc_logistic,acc_refined_logistic


def test_first_period(gnn_first_period, lr_model, n_classes, gen, iters=args.num_examples_test,
    ...
    # The accuracy we get from the spectral clustering
    acc_spectral_clustering_list = np.zeros([iters])

    acc_refined_list = np.zeros([iters])

    # The accuracy we get from using the logistic model

    acc_logsitic_list = np.zeros([iters])

    acc_refined_logsitic_list = np.zeros([iters])

    for it in range(iters):
        loss_test_first, acc_test_first, acc_spectral_clustering, acc_refined ,acc_logistic ,\
            acc_logistic_refined = test_single_first_period(
            gnn_first_period, lr_model, gen, n_classes, args, it, mode, class_sizes)
        ...
        acc_logsitic_list[it] = acc_logistic
        acc_refined_logsitic_list[it] = acc_logistic_refined
    ...
    acc_logsitic_avg_test_acc = np.mean(acc_logsitic_list)

    acc_refined_logsitic_avg_test_acc = np.mean(acc_refined_logsitic_list)
    ...
    df = pd.DataFrame([{
        ...
        "acc_logsitic_avg_test_acc" : acc_logsitic_avg_test_acc,
        "acc_refined_logsitic_avg_test_acc" : acc_refined_logsitic_avg_test_acc,
        "SNR": snr
    }])
    ...
    try:
        ...
        for class_sizes in class_sizes_list:
            ...
            for snr in snr_list:
                ...
                task_args.append((gnn_first_period, lr_model ,class_sizes, snr, gen, logN_div_N, total_ab))
